...
describe('GET:/playground/orgs/:orgKey/projects/:projectName/packageJSON', () => {
  let app
  let cache
  ...
  test('will retrieve the local package definition', async () => {
    const { body, headers, status } = await request(app)
      .get('/playground/orgs/orgA/projects/projectA01/packageJSON') // it reads weird, but this MUST go first
